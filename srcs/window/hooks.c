/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   hooks.c                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: vduchi <vduchi@student.42barcelona.com>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/27 19:51:11 by vduchi            #+#    #+#             */
/*   Updated: 2023/12/08 11:46:45 by vduchi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

/*
 * Keycode values:
 * 0   : A
 * 1   : S
 * 2   : D
 * 13  : W
 * 53  : ESC
 * 123 : Flecha izquierda
 * 124 : Flecha derecha
 */

#include "cub3d.h"
#include "structs.h"
#include <math.h>

int	exit_safe(int index)
{
	if (!index)
		ft_printf("Exit with ESC!\n");
	else
		ft_printf("Exit with the cross!\n");
	exit(0);
	return (0);
}

/*
void	arrows_pressed(int key, t_cube *cube)
{
	if (key == 123)
	{
		if (dbl_eq(cube->angle, 358.00))
			cube->angle = 0.00;
		else if (cube->angle + 2.00 > 360.00)
			cube->angle = 360.00 - cube->angle;
		else
			cube->angle += 2.00;
	}
	else if (key == 124)
	{
		if (dbl_eq(cube->angle, 0.00))
			cube->angle = 358.00;
		else if (cube->angle - 2.00 < 0.00)
			cube->angle = 360.00 - cube->angle;
		else
			cube->angle -= 2.00;
	}
}

void	update_pos(t_cube *cube, double *angle_vals, int sum, int idx)
{
	if (!sum)
	{
		cube->nposX += angle_vals[idx];
		cube->nposY -= angle_vals[idx + 1];
	}
	else
	{
		cube->nposX -= angle_vals[idx];
		cube->nposY += angle_vals[idx + 1];
	}
	if ((int)(cube->nposX / 64) > cube->iX)
		cube->iX++;
	else if ((int)(cube->nposX / 64) < cube->iX)
		cube->iX--;
	if ((int)(cube->nposY / 64) > cube->iY)
		cube->iY++;
	else if ((int)(cube->nposY / 64) < cube->iY)
		cube->iY--;
}

int	check_mov(t_cube *cube, int *idx_diff, int *idx_xy)
{
	printf("Angle: %f\tNO: %c\tSO: %c\tWE: %c\tEA: %c\n", cube->angle, cube->map[idx_diff[NO]][idx_xy[0]], cube->map[idx_diff[SO]][idx_xy[0]], cube->map[idx_xy[1]][idx_diff[WE]], cube->map[idx_xy[1]][idx_diff[EA]]);
	if ((dbl_eq(cube->angle, 0.00) || dbl_btw(cube->angle, 0.00, 90.00)) && (cube->map[idx_diff[NO]][idx_xy[0]] == '1' || cube->map[idx_xy[1]][idx_diff[EA]] == '1'))
		return (1);
	else if ((dbl_eq(cube->angle, 90.00) || dbl_btw(cube->angle, 90.00, 180.00)) && (cube->map[idx_diff[NO]][idx_xy[0]] == '1' || cube->map[idx_xy[1]][idx_diff[WE]] == '1'))
		return (1);
	else if ((dbl_eq(cube->angle, 180.00) || dbl_btw(cube->angle, 180.00, 270.00)) && (cube->map[idx_diff[SO]][idx_xy[0]] == '1' || cube->map[idx_xy[1]][idx_diff[WE]] == '1'))
		return (1);
	else if ((dbl_eq(cube->angle, 270.00) || dbl_btw(cube->angle, 270.00, 360.00)) && (cube->map[idx_diff[SO]][idx_xy[0]] == '1' || cube->map[idx_xy[1]][idx_diff[EA]] == '1'))
		return (1);
	return (0);
}

void set_idx(t_cube *cube, int *idx_diff, int *idx_xy, double *angle_vals)
{
	double	incr;

	incr = 10.00;
	if (cube->dir == 2)
		incr *= -1.00;
	idx_xy[0] = (int)fabs(cube->nposX / 64.00);
	idx_xy[1] = (int)fabs(cube->nposY / 64.00);
	idx_diff[NO] = (int)fabs((cube->nposY - incr) / 64.00);
	idx_diff[SO] = (int)fabs((cube->nposY + incr) / 64.00);
	idx_diff[WE] = (int)fabs((cube->nposX - incr) / 64.00);
	idx_diff[EA] = (int)fabs((cube->nposX + incr) / 64.00);
	angle_vals[COS] = 2.00 * cos(cube->angle * cube->rad_const);
	angle_vals[SIN] = 2.00 * sin(cube->angle * cube->rad_const);
	angle_vals[COS_90] = 2.00 * cos((cube->angle + 90.00) * cube->rad_const);
	angle_vals[SIN_90] = 2.00 * sin((cube->angle + 90.00) * cube->rad_const);
}

int	keep_pressed(int key, t_cube *cube)
{
	int		idx_xy[2];
	int		idx_diff[4];
	double	angle_vals[4];

//	printf("Keycode: %d\n", key);
	cube->mlx.status = 1;
	cube->dir = key + 1;
	set_idx(cube, idx_diff, idx_xy, angle_vals);
	if (key <= 13 && check_mov(cube, idx_diff, idx_xy))
		return (0);
	if (key == 0)
		update_pos(cube, angle_vals, 0, COS_90);
	else if (key == 1)
		update_pos(cube, angle_vals, 1, COS);
	else if (key == 2)
		update_pos(cube, angle_vals, 1, COS_90);
	else if (key == 13)
		update_pos(cube, angle_vals, 0, COS);
	else
		arrows_pressed(key, cube);
	return (0);
}
*/


void	arrows_pressed(int key, t_cube *cube)
{
	if (key == 123)
	{
//		if (dbl_eq(cube->angle, 358.00))
		if (cube->angle == 358.00)
			cube->angle = 0.00;
		else if (cube->angle + 2.00 > 360.00)
			cube->angle = 360.00 - cube->angle;
		else
			cube->angle += 2.00;
	}
	else if (key == 124)
	{
//		if (dbl_eq(cube->angle, 0.00))
		if (cube->angle == 0.00)
			cube->angle = 358.00;
		else if (cube->angle - 2.00 < 0.00)
			cube->angle = 360.00 - cube->angle;
		else
			cube->angle -= 2.00;
	}
}

void	update_pos(t_cube *cube, float *angle_vals, int sum, int idx)
{
	if (!sum)
	{
		cube->nposX += angle_vals[idx];
		cube->nposY -= angle_vals[idx + 1];
	}
	else
	{
		cube->nposX -= angle_vals[idx];
		cube->nposY += angle_vals[idx + 1];
	}
	if ((int)(cube->nposX / 64) > cube->iX)
		cube->iX++;
	else if ((int)(cube->nposX / 64) < cube->iX)
		cube->iX--;
	if ((int)(cube->nposY / 64) > cube->iY)
		cube->iY++;
	else if ((int)(cube->nposY / 64) < cube->iY)
		cube->iY--;
}

int	check_mov(t_cube *cube, int *idx_diff, int *idx_xy)
{
	printf("Angle: %f\tNO: %c\tSO: %c\tWE: %c\tEA: %c\n", cube->angle, cube->map[idx_diff[NO]][idx_xy[0]], cube->map[idx_diff[SO]][idx_xy[0]], cube->map[idx_xy[1]][idx_diff[WE]], cube->map[idx_xy[1]][idx_diff[EA]]);
//	if ((dbl_eq(cube->angle, 0.00) || dbl_btw(cube->angle, 0.00, 90.00)) && (cube->map[idx_diff[NO]][idx_xy[0]] == '1' || cube->map[idx_xy[1]][idx_diff[EA]] == '1'))
//		return (1);
//	else if ((dbl_eq(cube->angle, 90.00) || dbl_btw(cube->angle, 90.00, 180.00)) && (cube->map[idx_diff[NO]][idx_xy[0]] == '1' || cube->map[idx_xy[1]][idx_diff[WE]] == '1'))
//		return (1);
//	else if ((dbl_eq(cube->angle, 180.00) || dbl_btw(cube->angle, 180.00, 270.00)) && (cube->map[idx_diff[SO]][idx_xy[0]] == '1' || cube->map[idx_xy[1]][idx_diff[WE]] == '1'))
//		return (1);
//	else if ((dbl_eq(cube->angle, 270.00) || dbl_btw(cube->angle, 270.00, 360.00)) && (cube->map[idx_diff[SO]][idx_xy[0]] == '1' || cube->map[idx_xy[1]][idx_diff[EA]] == '1'))
//		return (1);
	if ((cube->angle == 0.00 || (cube->angle > 0.00 && cube->angle < 90.00)) && (cube->map[idx_diff[NO]][idx_xy[0]] == '1' || cube->map[idx_xy[1]][idx_diff[EA]] == '1'))
		return (1);
	else if ((cube->angle == 90.00 || (cube->angle > 90.00 && cube->angle < 180.00)) && (cube->map[idx_diff[NO]][idx_xy[0]] == '1' || cube->map[idx_xy[1]][idx_diff[WE]] == '1'))
		return (1);
	else if ((cube->angle == 180.00 || (cube->angle > 180.00 && cube->angle < 270.00)) && (cube->map[idx_diff[SO]][idx_xy[0]] == '1' || cube->map[idx_xy[1]][idx_diff[WE]] == '1'))
		return (1);
	else if ((cube->angle == 270.00 || (cube->angle > 270.00 && cube->angle < 360.00)) && (cube->map[idx_diff[SO]][idx_xy[0]] == '1' || cube->map[idx_xy[1]][idx_diff[EA]] == '1'))
		return (1);
	return (0);
}

void set_idx(t_cube *cube, int *idx_diff, int *idx_xy, float *angle_vals)
{
	float	incr;

	incr = 10.00;
	if (cube->dir == 2)
		incr *= -1.00;
	idx_xy[0] = (int)fabs(cube->nposX / 64.00);
	idx_xy[1] = (int)fabs(cube->nposY / 64.00);
	idx_diff[NO] = (int)fabs((cube->nposY - incr) / 64.00);
	idx_diff[SO] = (int)fabs((cube->nposY + incr) / 64.00);
	idx_diff[WE] = (int)fabs((cube->nposX - incr) / 64.00);
	idx_diff[EA] = (int)fabs((cube->nposX + incr) / 64.00);
	angle_vals[COS] = 2.00 * cos(cube->angle * cube->rad_const);
	angle_vals[SIN] = 2.00 * sin(cube->angle * cube->rad_const);
	angle_vals[COS_90] = 2.00 * cos((cube->angle + 90.00) * cube->rad_const);
	angle_vals[SIN_90] = 2.00 * sin((cube->angle + 90.00) * cube->rad_const);
}

int	keep_pressed(int key, t_cube *cube)
{
	int		idx_xy[2];
	int		idx_diff[4];
	float	angle_vals[4];

//	printf("Keycode: %d\n", key);
	cube->mlx.status = 1;
	cube->dir = key + 1;
	set_idx(cube, idx_diff, idx_xy, angle_vals);
	if (key <= 13 && check_mov(cube, idx_diff, idx_xy))
		return (0);
	if (key == 0)
		update_pos(cube, angle_vals, 0, COS_90);
	else if (key == 1)
		update_pos(cube, angle_vals, 1, COS);
	else if (key == 2)
		update_pos(cube, angle_vals, 1, COS_90);
	else if (key == 13)
		update_pos(cube, angle_vals, 0, COS);
	else
		arrows_pressed(key, cube);
	return (0);
}














int	esc_hook(int key)
{
	if (key == 53)
		exit_safe(0);
	return (0);
}

int	loop_hook(t_cube *cube)
{
	if (cube->mlx.status == 1)
	{
		calculate_rays(cube);
		create_minimap(cube);
		mlx_put_image_to_window(cube->mlx.mlx, cube->mlx.win, cube->mlx.img, 0, 0);
		cube->mlx.status = 0;
	}
	return (0);
}
