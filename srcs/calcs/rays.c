...
/*   Updated: 2023/12/10 12:45:06 by vduchi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	bigger_distance(t_cube *cube, t_rays *r, float angle)
{
	...
	{
		r->p_x = (float)((int)(fabs(cube->npos_y / 64.00)) + r->offset) *64.00;
	...
	{
		r->p_y = (float)((int)(fabs(cube->npos_x / 64.00)) + r->offset) *64.00;
...
{
	r->start_x = cube->npos_x;
	r->start_y = cube->npos_y;
	vals->diff = 0.00;
