...
/*   Updated: 2023/12/08 12:02:08 by vduchi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	if (!mode)
//	{
//		if (dbl_eq(angle, 0.00) || dbl_btw(angle, 0.00, 180.00))
...
//	else
//	{
//		if (dbl_eq(angle, 90.00) || dbl_btw(angle, 90.00, 270.00))
...
void	take_direction(t_cube *cube, t_rays *r, float angle, int idx)
{
...
//	print_screen(cube, cube->walls.dist);
	...
	else if (cube->angle + 30.00 > 360.00)
	{
		...
		angle = cube->angle - 30.0;
	while (diff != 0.00 && angle < 359.9999)
	{
		cube->walls.dist[idx] = bigger_distance(cube, &r, angle);
		cube->walls.dist[idx] *= cos(cos_angle * cube->rad_const);
		take_direction(cube, &r, angle, idx);
...
//			|| (!dbl_eq(diff, 0.00) && cube->angle < 30.00 && angle < 60.00 - diff)
//			|| (!dbl_eq(diff, 0.00) && cube->angle > 30.00 && angle < diff))
	while ((diff == 0.00 && cube->angle != 330.00 && angle < cube->angle + 30.00)
			|| (diff == 0.00 && cube->angle == 330.00 && angle != 0.00)
			|| (diff != 0.00 && cube->angle < 30.00 && angle < 60.00 - diff)
			|| (diff != 0.00 && cube->angle > 30.00 && angle < diff))
	{
		cube->walls.dist[idx] = bigger_distance(cube, &r, angle);
		cube->walls.dist[idx] *= cos(cos_angle * cube->rad_const);
		take_direction(cube, &r, angle, idx);
